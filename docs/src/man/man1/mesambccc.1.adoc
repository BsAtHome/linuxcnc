...
*writeflush* [Boolean]::
  ...
  initial values are propagated in Modbus write commands. Default true.
...
*name*::
  The base name for HAL pin names. +
  Example: if 'count="2"' and 'name="myname"', the the pins will have names
  like _myname-00_, _myname-01_, unless one or more '<pin>' tags override the
  name.
*noanswer* [Boolean]::
  Set to true if a device does not return a reply to a command. This can be
  intentional if you send a command to a non-existing device. Default false.
