...
*spiclk_rate* [default: 25000,-1,-1,-1,-1]::
  ...
  each probe takes the next value of the *spi_rate* list. A *spi_rate* or
  -1 automatically selects the first rate in the list. You may truncate
  the list to the number of boards you use.
*spiclk_rate_rd* [default: -1,-1,-1,-1,-1 (same as *spiclk_rate*)]::
...
The maximum SPI clock of the RP1-SPI driver and is documented over
33MHz. The SPI driver can provide frequencies well beyond what is
acceptable for any board. A safe value to start with would be 12.5 MHz
...
and the discrete SPI clock frequency (200 MHz / (2n) for n > 1):
...
It should be noted that the Raspberry Pi 5 *must* have an adequate 5V
power supply and the power should be properly decoupled. At high speeds
and noise on the supply, there is the possibility of strange behaviour
if the noise gets out of hand.

Also be sure to have a proper *active* heat-sink mounted on the SoC or it
will get too warm and crashes.
