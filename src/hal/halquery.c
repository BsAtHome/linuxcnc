...
static PyObject *get_comp(PyObject *self, PyObject *args)
{
    ...
    if(PyUnicode_Check(obj)) {
        rv = hal_comp_status_name(PyUnicode_AsUTF8(obj), &q);
    } else if(PyLong_Check(obj)) {
        rv = hal_comp_status_id((int)PyLong_AsLong(obj), &q);
    } else {
    ...
    default:
        PyErr_Format(PyExc_RuntimeError, "hal_comp_status_*: error=%d (%s)", rv, setps_errorstr(rv));
        return NULL;
    }
}
...
static PyObject *get_pin_param(PyObject *args, int ispin)
{
    ...
    default:
        PyErr_Format(PyExc_RuntimeError, "hal_get_p: error=%d (%s)", rv, setps_errorstr(rv));
        return NULL;
    }
}
...
static PyObject *get_signal(PyObject *self, PyObject *args)
{
    ...
    default:
        PyErr_Format(PyExc_RuntimeError, "hal_get_s: error=%d (%s)", rv, setps_errorstr(rv));
        return NULL;
    }
}
...
static PyObject *get_funct(PyObject *self, PyObject *args)
{
    ...
    default:
        PyErr_Format(PyExc_RuntimeError, "hal_list_funct: error=%d (%s)", rv, setps_errorstr(rv));
        return NULL;
    }
}
...
static int get_thread_cb(hal_query_t *q, void *arg)
{
    ...
    if(!strcmp(tlp->name, q->name)) {
        if(q->qtype == HAL_QTYPE_THREAD) {
            ...
            PyDict_SetItem(tlp->dict, PyUnicode_FromString(q->name), obj);
        } else {
            return -EINVAL;
        }
    } else if(tlp->dict) {
        // We moved to the next thread name and caught the one we want
        PyObject *obj = build_dict_thread(&tlp->qt, tlp->dict);
        tlp->dict = obj;
        return 1;
    }
    return 0;
...
static PyObject *get_thread(PyObject *self, PyObject *args)
{
    ...
    threadlist_t tl = { .name = name, .dict = NULL, .qt = {} };
    ...
    PyErr_Format(PyExc_RuntimeError, "hal_list_thread: error=%d (%s)", rv, setps_errorstr(rv));
    return NULL;
}
...
{
    PyDict_SetItem((PyObject *)arg, PyUnicode_FromString(q->name), build_dict_pin_param(q, 1));
    return 0;
}
...
static PyObject *get_signalpins(PyObject *self, PyObject *args)
{
    (void)self;
    hal_query_t q = {};
    if(!PyArg_ParseTuple(args, "s", &q.pp.signal))
        return NULL;
    ...
    PyErr_Format(PyExc_RuntimeError, "hal_list_p_s: error=%d (%s)", rv, setps_errorstr(rv));
    return NULL;
}
...
static PyObject *get_pins(PyObject *self, PyObject *args)
{
    ...
    if(!rv)
        return dict;
    PyErr_Format(PyExc_RuntimeError, "hal_list_p: error=%d (%s)", rv, setps_errorstr(rv));
    return NULL;
}
...
{
    PyDict_SetItem((PyObject *)arg, PyUnicode_FromString(q->name), build_dict_pin_param(q, 0));
    return 0;
}
...
{
    PyDict_SetItem((PyObject *)arg, PyUnicode_FromString(q->name), build_dict_signal(q));
    return 0;
}
...
static PyObject *get_signals(PyObject *self, PyObject *args)
{
    ...
    if(!rv)
        return dict;
    PyErr_Format(PyExc_RuntimeError, "hal_list_s: error=%d (%s)", rv, setps_errorstr(rv));
    return NULL;
}
...
{
    PyDict_SetItem((PyObject *)arg, PyUnicode_FromString(q->name), build_dict_comp(q));
    return 0;
}
...
static PyObject *get_comps(PyObject *self, PyObject *args)
{
    ...
    if(!rv)
        return dict;
    PyErr_Format(PyExc_RuntimeError, "hal_list_comp: error=%d (%s)", rv, setps_errorstr(rv));
    return NULL;
}
...
{
    PyDict_SetItem((PyObject *)arg, PyUnicode_FromString(q->name), build_dict_funct(q));
    return 0;
}
...
static int get_threads_cb(hal_query_t *q, void *arg)
{
    ...
    if(HAL_QTYPE_THREAD == q->qtype) {
        if(tcb->functs) {
            ...
            PyDict_SetItem(tcb->threads, PyUnicode_FromString(tcb->qt.name), build_dict_thread(&tcb->qt, tcb->functs));
            tcb->functs = NULL;
        }
        ...
        PyList_Append(tcb->functs, build_dict_threadfunct(q));
    } else {
        return -EINVAL;
    }
    return 0;
...
static PyObject *get_threads(PyObject *self, PyObject *args)
{
	...
	// wanted. Iteration stops by end-of-list and not by a name change.
        if(tc.functs) {
            PyDict_SetItem(tc.threads, PyUnicode_FromString(tc.qt.name), build_dict_thread(&tc.qt, tc.functs));
        }
        return tc.threads;
    }
    PyErr_Format(PyExc_RuntimeError, "hal_list_thread: error=%d (%s)", rv, setps_errorstr(rv));
