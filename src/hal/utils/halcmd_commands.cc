...
static int preflight_net_cmd_cb(hal_query_t *q, void *arg)
{
    ...
    net_cmd_data_t *ncd = (net_cmd_data_t *)q->callerdata.pval;
...
int do_setp_cmd(const char *name, const char *value)
{
    hal_query_t q = {};
    q.name = name;
    q.callerdata.ival = 0; // Indicate to callback this is a setp call
...
static std::string querydata_valuestr(hal_type_t type, const hal_query_value_t *v)
{
    switch(type) {
    case HAL_BOOL:
...
int do_sets_cmd(const char *name, const char *value)
{
    hal_query_t q = {};
    q.name = name;
    q.callerdata.ival = 1; // Indicate to callback this is a sets call
    int rv = hal_set_s(&q, setps_common_cb, (void *)value);
    if(rv) {
        halcmd_error("signal '%s': %s\n", name, setps_errorstr(rv));
    }
    return rv;
}
...
{
    if(COMPONENT_TYPE_USER == q->comp.type && q->comp.pid != q->callerdata.ival)
...
int do_unloadusr_cmd(const char *mod_name)
{
    ...
    if(!strcmp(mod_name, "all")) {
        q.callerdata.ival = (int)getpid();
        hal_list_comp(&q, do_unloadusr_cmd_cb, NULL);
    } else {
        int rv = hal_comp_status_name(mod_name, &q);
        if(!rv) {
            if(COMPONENT_TYPE_USER == q.comp.type && q.comp.pid != (int)getpid())
                kill(abs(q.comp.pid), SIGTERM);
        }
    }
    return 0;
...
static int unloadrt_cmd_cb(hal_query_t *q, void *arg)
{
    ...
    if(COMPONENT_TYPE_REALTIME == q->comp.type) {
...
{
    if(strcmp(mod_name, "all")) {
        int rv = hal_comp_status_name(mod_name, NULL);
        if(rv) {
            if(-ENOENT == rv) {
                halcmd_error("component '%s' is not loaded\n", mod_name);
            } else {
                halcmd_error("hal_comp_status_name on '%s' returned error=%d\n", mod_name, rv);
            }
            return rv;
        }
...
int do_unload_cmd(const char *mod_name) {
    if(strcmp(mod_name, "all") == 0) {
        ...
        if(q.comp.type == COMPONENT_TYPE_REALTIME)
            return do_unloadrt_cmd(mod_name);
        else
...
int do_waitusr_cmd(const char *comp_name)
{
    ...
    hal_query_t q = {};
    int rv = hal_comp_status_name(comp_name, &q);
    if (rv) {
        halcmd_info("component '%s' not found or already exited\n", comp_name);
        return 0;
    }
    if (q.comp.type != COMPONENT_TYPE_USER) {
    ...
    while(1) {
        ...
        if(0 != hal_comp_status_name(comp_name, NULL)) {
            break;
        }
    }
...
static int print_comp_info_cb(hal_query_t *q, void *arg)
{
    ...
    case COMPONENT_TYPE_REALTIME:
        halcmd_output(" %5d  %-4s  %-*s",
            q->comp.comp_id,
            q->comp.type == COMPONENT_TYPE_REALTIME ? "RT" : "User",
            HAL_NAME_LEN, q->name);
        if(q->comp.type == COMPONENT_TYPE_USER) {
    ...
    case COMPONENT_TYPE_OTHER: {
        ...
        int rv = hal_comp_status_id(q->comp.comp_id, &qc);
...
{
    const char **patterns = (const char **)arg;
    if(!tmatch(q->callerdata.ival, q->pp.type) || !match(patterns, q->name))
        return 0;
...
{
    const char **patterns = (const char **)arg;
    if(!tmatch(q->callerdata.ival, q->sig.type) || !match(patterns, q->name))
        return 0;
...
static int save_signals_cb(hal_query_t *q, void *arg)
{
    ...
    if(!(q->callerdata.ival && (q->sig.writers || q->sig.readers)))
...
{
    hal_query_t q = {};
    q.callerdata.ival = only_unlinked;
    fprintf(dst, "# signals\n");
...
static int save_links_cb(hal_query_t *q, void *arg)
{
    FILE *fp = (FILE *)arg;
    if(q->pp.signal) {
        const char *arrow_str = q->callerdata.ival ? data_arrow1(q->pp.dir) : "";
...
static int save_nets_3_outpins_cb(hal_query_t *q, void *arg)
{
    ...
    reinterpret_cast<save_nets_state_t *>(q->callerdata.pval)->state = 1;
    return 0;
}
...
{
    FILE *fp = (FILE *)arg;
    save_nets_state_t *st = reinterpret_cast<save_nets_state_t *>(q->callerdata.pval);
    fprintf(fp, " ");
    if(st->state) {
        fprintf(fp, "=> ");
        st->state = 0;
...
static int save_nets_3_cb(hal_query_t *q, void *arg)
{
    ...
    qp = {};
    qp.pp.signal = q->name;
    qp.callerdata.pval = &st; // Counts pins
    qp.pp.dir = HAL_OUT;
...
{
    FILE *fp = (FILE *)arg;
    const char *arrow_str = q->callerdata.ival ? data_arrow2(q->pp.dir) : "";
...
static int save_nets_01_cb(hal_query_t *q, void *arg)
{
    ...
    fprintf(fp, "newsig %s %s\n", q->name, data_type(q->sig.type));
    hal_query_t qp = {};
    qp.callerdata.ival = q->callerdata.ival; // Propagate arrow type
    qp.pp.signal = q->name;
