...
static int do_help_cmd(const char *command);
...
static const char *data_value(hal_type_t type, void *valptr);
static const char *data_value2(hal_type_t type, void *valptr);
...
pid_t hal_systemv_nowait(const char *argv[], connectionRecType *context) {
    pid_t pid;
    int n;
    static const char nakStr[] = "SET LOADRT NAK";
...
int hal_systemv(const char *argv[], connectionRecType *context) {
    pid_t pid;
    int status;
    int retval;
    static const char nakStr[] = "SET LOADRT NAK";
...
static int doLock(const char *command, connectionRecType *context)
{
	int retval = 0;
	static const char nakStr[] = "SET LOCK NAK";
...
static int doUnlock(const char *command, connectionRecType *context)
{
    int retval=0;
    static const char nakStr[] = "SET UNLOCK NAK";
...
static int doLinkpp(const char *first_pin_name, const char *second_pin_name, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET LINKPP NAK";

    hal_query_t p1 = {};
    hal_query_t p2 = {};
    p1.qtype = HAL_QTYPE_PIN;
    p1.name  = first_pin_name;
    p2.qtype = HAL_QTYPE_PIN;
    p2.name  = second_pin_name;
    int pin1 = hal_getref_p(&p1);
    int pin2 = hal_getref_p(&p2);
    /* check if the pins are there */
    if (0 != pin1 || 0 != pin2) {
        /* first pr second pin not found*/
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: pin '%s' not found\n",
                    linenumber, 0 != pin1 ? first_pin_name : second_pin_name);
        sockWriteError(nakStr, context);
        return -EINVAL; 
    }
    ...
    retval = hal_signal_new(first_pin_name, p1.pp.type);
...
int doLinkPS(const char *pin, const char *sig, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET NET NAK";
...
static int preflightNet(const char *signal, hal_sig_t *sig, const char *pins[], connectionRecType *context) 
{
...
int doNet(const char *signal, const char *pins[], connectionRecType *context) 
{
    hal_sig_t *sig;
    int i, retval;
    static const char nakStr[] = "SET NET NAK";
...
static int doLink(const char *pin, const char *sig, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET LINKPS NAK";
...
static int doNewsig(const char *name, const char *type, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET NEWSIG NAK";
...
static int doSetp(const char *name, const char *value, connectionRecType *context)
{
    static const char nakStr[] = "SET SETP NAK";
    hal_query_t query = {};
    query.name = name;
    query.callerdata.ival = 0; // Indicate to callback this is a setp call
    int rv = hal_set_p(&query, setps_common_cb, (void *)value);
    if(rv) {
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: (%d) parameter or pin '%s' %s\n",
                 linenumber, rv, name, setps_errorstr(rv));
        sockWriteError(nakStr, context);
    }
    return rv;
}

static int doSets(const char *name, const char *value, connectionRecType *context)
{
    static const char nakStr[] = "SET SETS NAK";
    hal_query_t query = {};
    query.name = name;
    query.callerdata.ival = 1; // Indicate to callback this is a sets call
    int rv = hal_set_s(&query, setps_common_cb, (void *)value);
    if(rv) {
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: (%d) signal '%s' %s\n",
                 linenumber, rv, name, setps_errorstr(rv));
        sockWriteError(nakStr, context);
    }
    return rv;
...
static int doAddf(const char *name, const char *thread, const char *parm, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET ADDF NAK";
...
static int doDelf(const char *name, const char *thread, connectionRecType *context)
{
    int retval;
    static const char nakStr[] = "SET DELF NAK";
...
{
    int retval;
    static const char nakStr[] = "SET START NAK";
...
{
    int retval;
    static const char nakStr[] = "SET STOP NAK";
...
static int doLoadRt(const char *mod_name, const char *args[], connectionRecType *context)
{
...
/*    snprintf(context->outBuf, sizeof(context->outBuf), "Setup argv %s %s %s", argv[0], argv[1], argv[2]);
    ...
    int rv = hal_comp_insmod_args(mod_name, cp1);
    if(rv) {
...
static int doDelsig(const char *mod_name, connectionRecType *context)
{
    SHMFIELD(hal_sig_t) next;
    int retval, retval1, n;
    hal_sig_t *sig;
    char sigs[MAX_EXPECTED_SIGS][HAL_NAME_LEN+1];
    static const char nakStr[] = "SET DELSIG NAK";
...
static int doUnload(const char *mod_name, connectionRecType *context)
{
    SHMFIELD(hal_comp_t) next;
    int retval, retval1, n, all;
    hal_comp_t *comp;
    char comps[64][HAL_NAME_LEN+1];
    static const char nakStr[] = "SET UNLOAD NAK";
...
void halcmd_error(const char *fmt, ...) { (void)fmt; }
void halcmd_info(const char *fmt, ...) { (void)fmt; }

pid_t hal_systemv_nowait(const char *argv[]) {
    pid_t pid;
    int n;
...
static int doLoadUsr(const char *args[])
{
	...
	retval = 0;
	    ...
	    /* check for program becoming ready */
            hal_query_t q = {};
            int rv = hal_comp_status_name(new_comp_name, &q);
            if(!rv && q.comp.ready) {
                ready = 1;
            }
	    /* pacify the user */
...
#if 0
static int doLoadUsr(const char *argv[])
{
...
static void getCompInfo(const char *pattern, connectionRecType *context)
{
    SHMFIELD(hal_comp_t) next;
    int len;
    hal_comp_t *comp;
...
static void getPinInfo(const char *pattern, int valuesOnly, connectionRecType *context)
{
	...
	if (valuesOnly == 0)  
	    ...
	    pin->name,
	    data_value2(pin->type, dptr),
	    comp->comp_id,
	    data_type(pin->type),
	    pin_data_dir(pin->dir));
	else
...
static void getSigInfo(const char *pattern, int valuesOnly, connectionRecType *context)
{
    SHMFIELD(hal_sig_t) next;
    int len;
    hal_sig_t *sig;
      ...
      if (strncmp(pattern, sig->name, len) == 0) {
        ...
        if (valuesOnly == 0)
          snprintf(context->outBuf, sizeof(context->outBuf), "SIGNAL %s  %s  %s", 
            sig->name,
	    data_value(sig->type, dptr),
	    data_type(sig->type));
	else
	  snprintf(context->outBuf, sizeof(context->outBuf), "SIGNALVAL %s %s", sig->name, data_value(sig->type, dptr));
	sockWrite(context);
        }
      next = sig->next_ptr;
      }
...
static void getParamInfo(const char *pattern, int valuesOnly, connectionRecType *context)
{
	...
	if (valuesOnly == 0)
	    ...
	    param->name,
            data_value(param->type, SHMPTR(param->data_ptr)),
            comp->comp_id, 
	    data_type(param->type),
            param_data_dir(param->dir));
	else
...
static void getFunctInfo(const char *pattern, connectionRecType *context)
{
...
static void getThreadInfo(const char *pattern, connectionRecType *context)
{
...
static int doSave(const char *type, const char *filename, connectionRecType *context)
{
    FILE *dst;
    static const char nakStr[] = "SET SAVE NAK";
...
static cmdResponseType getEcho(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getVerbose(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getEnable(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getConfig(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getCommMode(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getCommProt(const char *s, connectionRecType *context)
{
  (void)s;
...
static cmdResponseType getComps(const char *s, connectionRecType *context)
{
  if (s == NULL) 
    getCompInfo("", context);
  else
...
static cmdResponseType getPins(const char *s, connectionRecType *context)
{
  if (s == NULL) 
...
static cmdResponseType getPinVals(const char *s, connectionRecType *context)
...
static cmdResponseType getSignals(const char *s, connectionRecType *context)
{
  if (s == NULL)
...
static cmdResponseType getSignalVals(const char *s, connectionRecType *context)
{
  if (s == NULL)
...
static cmdResponseType getParams(const char *s, connectionRecType *context)
{
  if (s == NULL)
...
static cmdResponseType getParamVals(const char *s, connectionRecType *context)
{
  if (s == NULL) 
...
static cmdResponseType getFuncts(const char *s, connectionRecType *context)
{
  if (s == NULL)
...
static cmdResponseType getThreads(const char *s, connectionRecType *context)
{
  if (s == NULL)
...
static cmdResponseType getComp(const char *s, connectionRecType *context)
{
...
static cmdResponseType getPin(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getPinInfo(s, 0, context);
  return rtHandledNoError;
...
static cmdResponseType getPinVal(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getPinInfo(s, 1, context);
  return rtHandledNoError;
...
static cmdResponseType getSignal(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getSigInfo(s, 0, context);
  return rtHandledNoError;
...
static cmdResponseType getSignalVal(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getSigInfo(s, 1, context);
  return rtHandledNoError;
...
static cmdResponseType getParam(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getParamInfo(s, 0, context);
  return rtHandledNoError;
...
static cmdResponseType getParamVal(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getParamInfo(s, 1, context);
  return rtHandledNoError;
...
static cmdResponseType getFunct(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getFunctInfo(s, context);
  return rtHandledNoError;
...
static cmdResponseType getThread(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
  getThreadInfo(s, context);
  return rtHandledNoError;
...
static int checkOnOff(const char *s)
{
  ...
  if (s == NULL) return -1;
//  strupr(s);
  if (strcasecmp(s, onStr) == 0) return 0;
  if (strcasecmp(s, offStr) == 0) return 1;
  return -1;
}

static int checkBinaryASCII(const char *s)
{
  ...
  if (s == NULL) return -1;
//  strupr(s);
  if (strcasecmp(s, ASCIIStr) == 0) return 0;
  if (strcasecmp(s, binaryStr) == 0) return 1;
  return -1;
}

static cmdResponseType setEcho(const char *s, connectionRecType *context)
{
...
static cmdResponseType setVerbose(const char *s, connectionRecType *context)
{
...
static cmdResponseType setEnable(const char *s, connectionRecType *context)
{
...
static cmdResponseType setConfig(const char *s, connectionRecType *context)
{
  (void)s;
  (void)context;
  return rtNoError;
...
static cmdResponseType setCommMode(const char *s, connectionRecType *context)
{
  int ret;
...
static cmdResponseType setCommProt(const char *s, connectionRecType *context)
{
  (void)s;
  char *pVersion;
...
static cmdResponseType setLoadRt(const char *s, connectionRecType *context)
{
  char *pch;
  const char *args[MAX_TOK+3];
  int i;
...
static cmdResponseType setUnload(const char *s, connectionRecType *context)
{
...
static cmdResponseType setLoadUsr(const char *s, connectionRecType *context)
{
  (void)context;
  const char *argv[2];
  argv[0] = s;
...
static cmdResponseType setLinkps(const char *p, const char *s, connectionRecType *context)
{
...
static cmdResponseType setLinksp(const char *p, const char *s, connectionRecType *context)
{
  if ((p == NULL) || (s == NULL)) return rtStandardError;
...
static cmdResponseType setLinkpp(const char *p1, const char *p2, connectionRecType *context)
{
...
static cmdResponseType setNet(const char *p1, connectionRecType *context)
{
  const char *argv[MAX_TOK];
  int i;
  char *pch;
...
static cmdResponseType setUnlink(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
...
static cmdResponseType setLock(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
...
static cmdResponseType setUnlock(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
...
static cmdResponseType setNewsig(const char *s, const char *t, connectionRecType *context)
{
...
static cmdResponseType setDelsig(const char *s, connectionRecType *context)
{
  if (s == NULL) return rtStandardError;
...
static cmdResponseType setSetp(const char *s, const char *p, connectionRecType *context)
{
...
static cmdResponseType setSets(const char *s, const char *p, connectionRecType *context)
{
  if ((s == NULL) || (p == NULL)) return rtStandardError;
...
static cmdResponseType setAddf(const char *s, const char *t, const char *p, connectionRecType *context)
{
  if ((s == NULL) || (t == NULL)) return rtStandardError;
...
static cmdResponseType setDelf(const char *s, const char *t, connectionRecType *context)
{
  if ((s == NULL) || (t == NULL)) return rtStandardError;
...
static cmdResponseType setSave(const char *type, const char *fileName, connectionRecType *context)
{
...
commandTokenType lookupToken(const char *s)
{
