...
//
// It expects the q->callerdata.ival to be set to allow error signalling of the
// HAL_PORT type that is not allowed in signals. Value should be:
//   0 - pin/param
//   1 - signal
//
...
//   -ENOTSUP - setting of q->pps.type not supported
//   -ENOMEM  - failedto allocate memory for locale switching
//
...
//
// Return an appropriate error string for setp/sets error values
//
// The error value should be a negative errno value returned by hal_[gs]et_p()
// or hal_[gs]et_s().
//
const char *setps_errorstr(int err)
{
    switch(err) {
    ...
    case -ENOTSUP: return "type not supported";
    case -EBADF:   return "invalid type";
    case -ERANGE:  return "value out of range";
    case -ENOMEM:  return "out of memory";
    case -EEXIST:  return "queue already allocated";
