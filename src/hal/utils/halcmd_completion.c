...
static int match_pintype_cb(hal_query_t *q, void *arg)
{
    ...
    if (q->callerdata.uval == strlen(q->name) && !strncmp(name, q->name, q->callerdata.uval)) {
...
static void check_match_type_pin(const char *name)
{
    ...
    q.callerdata.uval = strcspn(name, " \t");
...
{
    gendata_add((gendata_list_t *)arg, q->name, q->callerdata.ival);
    return 0;
}
...
static char *usrcomp_generator(const char *text, int state)
{
    ...
    for(; idx < gdl.n; idx++) {
        if(gdl.data[idx].ctype != COMPONENT_TYPE_USER)
            continue;
	if(!strncmp(text, gdl.data[idx].name, len)) {
            return strdup(gdl.data[idx++].name);
        }
...
static char *rtcomp_generator(const char *text, int state)
{
    ...
    for(; idx < gdl.n; idx++) {
        if(gdl.data[idx].ctype != COMPONENT_TYPE_REALTIME)
            continue;
	if(!strncmp(text, gdl.data[idx].name, len)) {
            return strdup(gdl.data[idx++].name);
        }
