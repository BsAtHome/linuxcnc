...
static void update(void *arg, long period)
{
    ...
    for (int n = 0; n < num_pins; n++) {
        ...
        case HAL_FLOAT: hal_set_real(str->pins[n].r, data[n].f); break;
        case HAL_S32:   hal_set_si32(str->pins[n].s, data[n].s); break;
        case HAL_U32:   hal_set_ui32(str->pins[n].u, data[n].u); break;
        case HAL_S64:   hal_set_sint(str->pins[n].s, data[n].l); break;
        case HAL_U64:   hal_set_uint(str->pins[n].u, data[n].k); break;
        case HAL_BIT:   hal_set_bool(str->pins[n].b, data[n].b); break;
        default:
            break;
        }
    }
...
static int init_streamer(int num, streamer_t *str)
{
    ...
    for (int n = 0; n < hal_stream_element_count(&str->fifo); n++) {
        ...
        case HAL_FLOAT: retval = hal_pin_new_real(comp_id, HAL_OUT, &str->pins[n].r, 0.0, pstr, num, n); break;
        ...
        case HAL_S64:   retval = hal_pin_new_sint(comp_id, HAL_OUT, &str->pins[n].s, 0,   pstr, num, n); break;
        case HAL_U64:   retval = hal_pin_new_uint(comp_id, HAL_OUT, &str->pins[n].u, 0,   pstr, num, n); break;
        case HAL_BIT:   retval = hal_pin_new_bool(comp_id, HAL_OUT, &str->pins[n].b, 0,   pstr, num, n); break;
        default:
