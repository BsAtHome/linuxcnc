...
//
// NOTE: The hal_setX API is not exported in the kernel. It is a user-land
// thing and should only be called from non-RT applications.
//
static int set_common(hal_type_t type, const hal_query_value_t *v, hal_refs_u u, bool setport)
{
    switch(type) {
    ...
    case HAL_PORT: {
        ...
        int rv = hal_port_alloc(v->u, (hal_port_t *)u.s);
        if(rv)
            return rv;
        } break;
    default:
...
int hal_set_p(hal_query_t *q, hal_query_cb cb, void *arg)
{
    ...
    if(!pin) {
        ...
        if(q->pp.type > 0 && q->pp.type != param->type) {
        ...
        q->pp.dir    = param->dir;
        q->pp.alias  = param->oldname ? (const char *)SHMPTR(param->oldname) : NULL;
        q->pp.signal = NULL;
        ...
        if(q->pp.type > 0 && q->pp.type != pin->type) {
            // Specific type requested and it didn't match
            halpr_mutex_release();
            return -ENOENT;
        }
        // We have the pin, copy the data, but we still can fail
        q->qtype     = HAL_QTYPE_PIN;
        q->name      = pin->name;
        q->pp.type   = t = pin->type;
        q->pp.ref    = (hal_refs_u)(hal_sint_t)&pin->dummysig;
        q->pp.dir    = pin->dir;
        q->pp.alias  = pin->oldname ? (const char *)SHMPTR(pin->oldname) : NULL;
        q->pp.signal = NULL;
...
static int get_common(hal_type_t type, hal_query_value_t *v, hal_refs_u u, bool getport)
{
    switch(type) {
...
int hal_get_s(hal_query_t *q, hal_query_cb cb, void *arg)
{
    ...
    if(q->sig.type > 0 && q->sig.type != sig->type) {
...
//
// Calls the callback with the mutex *locked*.
//
int hal_list_p(hal_query_t *q, hal_query_cb cb, void *arg)
{
    ...
    if(!q || !cb || (q->qtype && !(HAL_QTYPE_PIN == q->qtype || HAL_QTYPE_PARAM == q->qtype))) {
...
// Expects the signal name reference in q->pp.signal
// You can filter on direction in q->pp.dir
//
int hal_list_p_s(hal_query_t *q, hal_query_cb cb, void *arg)
{
    ...
    if(0 != q->pp.dir && 0 != (q->pp.dir & ~HAL_IO)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_list_p_s: Invalid query dir argument\n");
        return -EINVAL;
    }

    hal_pdir_t dir = q->pp.dir;

    // Grab the mutex to keep the data stable
    halpr_mutex_acquire();

    hal_sig_t *sig = halpr_find_sig_by_name(q->pp.signal);
    if(!sig) {
        halpr_mutex_release();
    ...
    while(NULL != pin) {
        if(0 == dir || pin->dir == dir) {
            ...
            q->pp.comp_id    = comp->comp_id;
            q->pp.ref        = (hal_refs_u)(hal_sint_t)SHMPTR(sig->data_ptr);
            // Was set by caller, but rewrite if callback changes it
            q->pp.signal = sig->name;
...
int hal_list_s(hal_query_t *q, hal_query_cb cb, void *arg)
{
	...
	// hal_list_p_s() o get the full list.
        q->sig.ref     = (hal_refs_u)(hal_sint_t)SHMPTR(sig->data_ptr);
        if(0 != (rv = cb(q, arg))) {
            // Non-zero return from the callback breaks the loop and we're done
            break;
...
{
    if(NULL == hal_data) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_comp_status_id: HAL shared memory not mapped\n");
        return -EFAULT;
    }
    if(comp_id <= 0 || comp_id > 0xffff) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_comp_status_id: Invalid query arguments\n");
        return -EINVAL;
    }
    // Grab the mutex to keep the data stable
    halpr_mutex_acquire();
    hal_comp_t *comp = halpr_find_comp_by_id(q->comp.comp_id);
...
{
    if(NULL == hal_data) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_comp_status_name: HAL shared memory not mapped\n");
        return -EFAULT;
    }
    if(!name) {
        return -EINVAL;
