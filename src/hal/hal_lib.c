...
// not exposed. This is used in the stepgen component.
...
int hal_create_thread(const char *name, unsigned long period_nsec, int uses_fp)
{
    ...
    if (hal_param_new_si32(new->comp_id, HAL_RW, &(new->maxtime), 0, "%s.tmax", new->name) < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
           "HAL: ERROR: fail to create param '%s.tmax'\n", new->name);
        return -EINVAL;
    }

    if (hal_pin_new_si32(new->comp_id, HAL_OUT, &(new->runtime), 0, "%s.time", new->name) < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
    ...
    if(hal_param_new_sint(new->comp_id, HAL_RO, &(new->threadbeat), 0, "%s.threadbeat", name) < 0) {
