...
static PyObject *pyhal_read_common(halitem *item) {
    ...
    case HAL_PORT:
        if(item->is_pin)
            return to_python(hal_port_buffer_size((hal_port_t *)item->u->u));
        else
...
PyObject *pin_has_writer(PyObject * /*self*/, PyObject *args) {
    ...
    PyErr_Format(PyExc_RuntimeError, "pin_has_writer: %s: returned error: %s", q.name, setps_errorstr(rv));
    return NULL;
}
...
PyObject *component_exists(PyObject * /*self*/, PyObject *args) {
    ...
    int rv = hal_comp_status_name(name, NULL);
    if(0 == rv) {
        Py_INCREF(Py_True);
        return Py_True;
    } else if(-ENOENT == rv) {
    ...
    PyErr_Format(PyExc_RuntimeError, "component_exists: hal_comp_status('%s') returned %d", name, rv);
    return NULL;
}
...
PyObject *component_is_ready(PyObject * /*self*/, PyObject *args) {
    ...
    hal_query_t q;
    int rv = hal_comp_status_name(name, &q);
...
static int set_common_cb(hal_query_t *q, void *arg)
{
    ...
    hal_query_value_t *qvp = HAL_QTYPE_SIGNAL == q->qtype ? &q->sig.value : &q->pp.value;
...
PyObject *set_p(PyObject * /*self*/, PyObject *args) {
    ...
    if(rv < 0) {
        PyErr_Format(PyExc_RuntimeError, "set_p: %s: %s", q.name, setps_errorstr(rv));
        return NULL;
    }
...
PyObject *set_s(PyObject * /*self*/, PyObject *args) {
    ...
    if(rv < 0) {
        PyErr_Format(PyExc_RuntimeError, "set_s: %s: %s", q.name, setps_errorstr(rv));
        return NULL;
    }
    return PyBool_FromLong(1);
}
...
static PyObject *halref_to_object(hal_type_t type, const hal_query_value_t *v)
{
    switch(type) {
...
PyObject *get_p(PyObject * /*self*/, PyObject *args)
{
    ...
    PyErr_Format(PyExc_RuntimeError, "get_p: %s: %s", q.name, setps_errorstr(rv));
    return NULL;
}
...
PyObject *get_s(PyObject * /*self*/, PyObject *args)
{
    ...
    PyErr_Format(PyExc_RuntimeError, "get_s: %s: %s", q.name, setps_errorstr(rv));
    return NULL;
}
...
PyObject *get_value(PyObject * /*self*/, PyObject *args)
{
    ...
    PyErr_Format(PyExc_RuntimeError, "get_value: %s: %s", q.name, setps_errorstr(rv));
    return NULL;
}
...
PyObject *get_info_pins(PyObject * /*self*/, PyObject * /*args*/)
{
    ...
    if(0 != rv) {
        ...
        PyErr_Format(PyExc_RuntimeError, "hal_list_p: returned '%s' (%d)", rv, setps_errorstr(rv));
        return NULL;
    }
    return python_list;
}
...
PyObject *get_info_signals(PyObject * /*self*/, PyObject * /*args*/)
{
    ...
    if(0 != rv) {
        Py_DECREF(python_list);
        PyErr_Format(PyExc_RuntimeError, "hal_list_s: returned '%s' (%d)", rv, setps_errorstr(rv));
        return NULL;
    }
    return python_list;
}
...
static int hal_module_exec(PyObject *m)
{
    if(NULL != pyhal_error_type) {
        PyErr_SetString(PyExc_ImportError, "Cannot initialize _hal module more than once");
        return -1;
    }
...
static PyModuleDef_Slot hal_module_slots[] = {
    {Py_mod_exec, (void *)hal_module_exec},
    {0, NULL}
};
...
{
    return PyModuleDef_Init(&hal_moduledef);
}
