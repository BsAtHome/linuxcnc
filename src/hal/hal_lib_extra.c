...
{
    if(NULL == hal_data) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_enforce_exact_base_period: HAL shared memory not mapped\n");
        return -EFAULT;
    }
    halpr_mutex_acquire();
...
{
    if(NULL == hal_data) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_invoke_make: HAL shared memory not mapped\n");
        return -EFAULT;
    }
    if(!compname || !newname || !arg) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_invoke_make: Invalid arguments\n");
        return -EINVAL;
    }
...
{
    if(NULL == hal_data) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_comp_insmod_args: HAL shared memory not mapped\n");
        return -EFAULT;
    }
    if(!compname || !args) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_comp_insmod_args: Invalid arguments\n");
        return -EINVAL;
    }
    if(!SHMCHK(args)) {
