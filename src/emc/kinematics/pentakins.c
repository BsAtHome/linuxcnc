...
//  PmRpy q_RPY;
  ...
  while (iterate) {
    ...
    for (i = 0; i < NUM_STRUTS; i++) {
      if (fabs(StrutLengthDiff[i]) > hal_get_real(haldata->conv_criterion)) {
    iterate = 1;
      }
    }
...
int rtapi_app_main(void)
{
    ...
    for (i = 0; i < NUM_STRUTS; i++) {
        ...
        if ((res = hal_pin_new_real(comp_id, HAL_RW, &(haldata->basex[i]),
                                    init_basex[i], "pentakins.base.%d.x", i)) < 0)
            goto error;

        if ((res = hal_pin_new_real(comp_id, HAL_RW, &haldata->basey[i],
                                    init_basey[i], "pentakins.base.%d.y", i)) < 0)
            goto error;

        if ((res = hal_pin_new_real(comp_id, HAL_RW, &haldata->basez[i],
                                    init_basez[i], "pentakins.base.%d.z", i)) < 0)
            goto error;

        if ((res = hal_pin_new_real(comp_id, HAL_RW, &haldata->effectorr[i],
                                    init_effectorr[i], "pentakins.effector.%d.r", i)) < 0)
            goto error;

        if ((res = hal_pin_new_real(comp_id, HAL_RW, &haldata->effectorz[i],
